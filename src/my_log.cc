...
namespace my
{
  ...
  static void _log_on_level(int curr_level, const char *buf)
  {
    ...
    if (curr_level == MYLOG_LEVEL_WARNING)
      prefix = "[warning]";
    else if (curr_level == MYLOG_LEVEL_ERROR)
      prefix = "[error]";
    else if (curr_level == MYLOG_LEVEL_FATAL)
      prefix = "[fatal]";
    else if (curr_level == MYLOG_LEVEL_DEBUG)
      prefix = "[debug]";
    else
      prefix = "";
  ...
  int init_logger(int level, int type, const char *log_path, bool nosign)
  {
    ...
    if (level < MYLOG_LEVEL_NONE)
      level = MYLOG_LEVEL_NONE;
    if (level > MYLOG_LEVEL_DEBUG)
      level = MYLOG_LEVEL_DEBUG;
    g_debug_level = level;

    if (type < MYLOG_TYPE_STDOUT)
      type = MYLOG_TYPE_STDOUT;
    if (type > MYLOG_TYPE_BOTH)
      type = MYLOG_TYPE_BOTH;
    g_debug_type = type;
